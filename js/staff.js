// ── staff.js ── (Teacher: places high-priority orders)
import {
...
document.querySelectorAll('input[name="deliveryMethod"]').forEach(r => r.addEventListener('change', () => {
...
// ── Price calculator ──
document.querySelectorAll('input[name="colourMode"], input[name="sideMode"]').forEach(el => el.addEventListener('change', updatePrice));
['copies', 'pages', 'docName', 'block', 'roomNo'].forEach(id => {
...
function updatePrice() {
  ...
  const discount       = (pages * copies) > s.bulkThreshold ? s.bulkPercent : 0;
...
function validForm() {
  ...
  if (isCampus) {
    if (!document.getElementById('block').value.trim()) return false;
    if (!document.getElementById('roomNo').value.trim()) return false;
  }
  return true;
}
...
document.getElementById('orderForm').addEventListener('submit', e => {
  ...
  const { subtotal, discount, deliveryFee, total } = updatePrice();
  const isCampus = document.querySelector('input[name="deliveryMethod"]:checked')?.value === 'campus';
  ...
  toast(`⚡ Priority order placed! Token: ${token}`, 'success');
...
function renderMyOrders() {
  ...
  if (!filtered.length) {
    tbody.innerHTML = '<tr><td colspan="9" class="muted small" style="padding:20px;text-align:center">No orders found.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = filtered.map(o => {
    ...
    const cancelBtn = o.printStatus === 'pending'
      ...
      : '';
    return `<tr class="priority-row">
      <td>${priorityBadge('high')}</td>
